...
breakAt :: String -> String -> [String]
breakAt _ [] = []
breakAt a (x:xs)
  | pack a == pack [x] = []:breakAt a xs
  | otherwise = (x:y):ys
  where
    (y:ys) = breakAt a xs
...
{-# INLINABLE mkPolicy #-}
mkPolicy :: String -> PubKeyHash -> () -> ScriptContext -> Bool
...
mkWrappedPolicy :: String -> PubKeyHash -> BuiltinData -> BuiltinData -> ()
...
-- use bytestrings for this entire flow. 
-- utf8 encode it.  breakAt should be in plutusTx - sliceByteString. 